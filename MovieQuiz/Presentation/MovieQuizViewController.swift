...
final class MovieQuizViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let currentQestion = questions[currntQuestionIndex]
        let step = convert(model: currentQestion)
        show(quiz: step)
    }
    ...
    private func convert(model: QuizQuestion) -> QuizStepViewModel {
        let questionStep = QuizStepViewModel(
            ...
            question: model.text,
            questionNumber: "\(currntQuestionIndex + 1)/\(questions.count)"
        )
        return questionStep
    }
    ...
    private func showNextQestionOrResults() {
        if currntQuestionIndex == questions.count - 1 {
            ...
            let nextQuestion = questions[currntQuestionIndex]
    ...
    private func show(quiz result: QuizResultsViewModel) {
        ...
        let action = UIAlertAction(title: result.buttonText, style: .default) { _ in
            ...
            let firstQuestion = self.questions[self.currntQuestionIndex]
